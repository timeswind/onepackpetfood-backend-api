...
const config = {
    ...
    Proxy: '',
    SecretId: require('config-lite').tencent_cloud_SecretId, // 固定密钥
    SecretKey: require('config-lite').tencent_cloud_SecretKey, // 固定密钥
    Bucket: 'xiaoquanju-1257075795',
    Region: 'ap-shanghai',
...
exports.get = function* () {
    // 获取前端过来的参数
    var method = this.query.method
    var pathname = this.query.pathname
    // 获取临时密钥，计算签名
    var getTempKeys = yield getTempKeysAsync()
    if (getTempKeys.success) {
        tempKeys = getTempKeys.data
        data = {
