...
exports.getAllInactive = function () {
  return Tagtrack.find({active: false, type: 'store'}).sort({ _id: -1 }).exec();
};

exports.activeTagtrack = function(code) {
  return Tagtrack.findOneAndUpdate({ "code": code }, { "$set": {active: true, active_date: new Date()} }, { new: true }).exec()
}
